...
const DashPosts = () => {
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await fetch(`/post/getposts?userId=${currentUser._id}&startIndex=${startIndex}`)
      const data = await res.json()
      if(res.ok){
        ...
        <Modal.Header/>
          <Modal.Body>
            <div className='text-center'>
              ...
              mb-4 mx-auto'/>
              <h3 className='mb-5
              text-lg text-gray-500 dark:text-gray-400'>Are you sure you want to delete this post?</h3>
